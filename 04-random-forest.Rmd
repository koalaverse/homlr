...
```{block, type="rmdnote"}
To produce a PDP with multi-nomial classification problems, we need to create a custom prediction function that will return a data frame of the ___mean predicted probability___ for each response class. We supply this custom prediction function to the `pdp::partial` function call.
```
...
```{r h2o-rf-multinomial-class-init, cache=FALSE}
h2o::h2o.no_progress()
h2o.init(max_mem_size = "5g")
```

```{r h2o-rf-multinomial-remove, include=FALSE, eval=FALSE}
h2o.removeAll()
```
...
```{r h2o-rf-class-multinomial-data-prep, eval=FALSE}
# convert training data to h2o object
train_h2o <- train %>%
...
```{block, type="rmdtip"}
This grid search ran for 3 hours and only evaluated 6 models!
```
...
```{r rf-h2o-tuned-confusion-matrix, eval=FALSE}
# grab top model id
...
```{r rf-h2o-multi-class-final-fit, eval=FALSE}
...
```{r rf-h2o-multi-classification-best-model-import, echo=FALSE, cache=FALSE}
# save the model
#h2o.saveModel(object = h2o_final, path = "outputs/h2o_best_rf_model", force = TRUE)

# import model
h2o_final <- h2o.loadModel(path = "outputs/h2o_best_rf_model/DRF_model_R_1533647638822_3")
```
...
```{r rf-multi-classification-h2o-top25-vip, fig.width=10, fig.cap="Top 25 most important variables based on impurity."}
...
```{r rf-h20-multi-classification-pdp, eval=FALSE}
# custom prediction function
custom_pred <- function(object, newdata) {
...
```{r rf-h20-multi-classification-pdp-plot, echo=FALSE, fig.cap="Partial dependence plots of our most influential variable (`V407`) across the 10 response levels. This variable appears to be most influential in predicting the number 0, 1, 7, and 8."}
knitr::include_graphics("images/rf-h20-multi-classification-pdp.png")
```
...
```{block, type="rmdtip"}
Note the Top-10 Hit Ratios.  With our regularized regression __h2o__ model, it took 4 "guesses" or tries until we achieved a 99% accuracy rate.  With our random forest model, it only takes 2 tries for our model to accurately predict 99% on our test data!
```


```{r rf-h20-multi-classification-final-predictions, eval=FALSE}
